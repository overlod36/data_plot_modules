...
def find_centroid_point(cluster_points: list, calculated_point: list) -> int:
    distations = [eucl_dist(cl_point[:2], calculated_point) for cl_point in cluster_points]
    return distations.index(min(distations))

def clusters_formation(points: list, central_points: list) -> list:
    keep_point = True
    while keep_point:
        keep_point = False
        for point in points:
            distations = [eucl_dist(points[c_point], point) for c_point in central_points]
        ...
        for c_point_index in central_points:
            ...
            final_point_index = find_centroid_point(cluster_points, calculated_point)
